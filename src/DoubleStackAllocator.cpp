...
void* DoubleStackAllocator::AllocateLower(size_t sizeInBytes)
{
	...
	if (availableSize >= sizeInBytes)
	{
		Marker newMarker = m_LowerMarker + sizeInBytes;
		uintptr_t assignedAddress = reinterpret_cast<uintptr_t>(m_AllocatedMemory) + newMarker;
...
void DoubleStackAllocator::ClearLower()
{
	m_LowerMarker = 0;
}

void DoubleStackAllocator::ClearUpper()
{
	m_UpperMarker = m_StackSizeInBytes;
}
